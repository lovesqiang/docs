...
尚硅谷 VibeCoding 实战教程：https://www.bilibili.com/video/BV1RPET6tEp2/
:::
...
终端（Terminal）是AI编程的控制台，几乎所有AI工具都在终端中运行。
...
**传统编程**：人写代码，关注“How”（怎么做）。

**AI辅助编程**：人用自然语言描述需求，AI生成代码，关注“What & Why”（做什么、为什么）。

**核心转变**：从代码编写者 → 需求定义者 + 结果审查者

#### 1.1.2 AI编程的发展历程

```
第一阶段     第二阶段      第三阶段           第四阶段
代码补全  →  聊天式AI  →  Agent（当前）  →   多Agent协作（未来）
```

#### 1.1.3 AI是如何"理解"和"生成"代码的

**Token：**AI把文本切成小块，计费和能力限制以Token为单位。

例如，`Hello World` 会被切成 `Hello` 和 ` World` 两个 Token。中文的 `你好世界` 可能被切成 `你好` 和 `世界` 两个 Token，也可能被切成更多个。

> 1 个 Token ≈ 4 个英文字符 ≈ 1-2 个中文字符。
...
这就是为什么我们把 cc 称为 **Agent（智能体）** 而不是 Chatbot（聊天机器人）。
...
Harness 的 7 层扩展点——下三层是“纪律”（项目上下文、钩子、知识），上四层是“武器”（包分发、IDE、外部、子代理）*
...
官方也提供原生安装器，用于不想通过 npm 管理 Claude Code 的用户。
...
**国内用户** 如果官方安装器网络不稳定，可以临时使用 npm 方式，并先配置 npm 镜像：
